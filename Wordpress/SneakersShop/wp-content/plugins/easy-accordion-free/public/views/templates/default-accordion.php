...
foreach ( $content_sources as $key => $content_source ) {
	...
	global $wp_embed;
	$content_description = do_shortcode( $wp_embed->autoembed( $content_embed ) );
	...
	if ( ! empty( $content_description ) ) {
		...
		echo '<div class="ea-card ' . $expand_class . ' ' . $accordion_item_class . '">';
			echo sprintf(
				'<h3 class="ea-header"><a class="collapsed" data-sptoggle="spcollapse" %1$s href="javascript:void(0)">%2$s %3$s</a></h3>',
				$data_sptarget,
				$eap_icon_markup,
				$content_title
			);
