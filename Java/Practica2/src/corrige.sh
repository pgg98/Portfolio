...
for fichero in $fuentes; do
   if test $fichero == Tipo.java || test $fichero == Objeto.java || \
      ...
      test $fichero == Emisor.java || test $fichero == Influenciable.java ||\
      test $fichero == Celestial.java || test $fichero == Diablo.java || \
      test $fichero == Querube.java ; then 
...
if test $numfuentes -eq 9; then
  if test $numero -gt 9; then
  ...
  continuar=false
  echo "Error, los ficheros fuente necesarios son Tipo.java, Objeto.java, Persona.java, Tienda.java, Emisor.java, Influenciable.java, Celestial.java, Diablo.java y Querube.java; 0"
fi
if $continuar; then
  abstracta=$(grep "abstract" Celestial.java |wc -l)
  if test $abstracta -eq 3; then
    continuar=true;
  else
    continuar=false;
    echo "Error, la clase Celestial tiene que ser abstracta; 0"
  fi
fi
if $continuar; then
